...
const testArrayStrings = ['123', 'first ', "don't judge book by it's cover", '9'];

type FilterCallBack<T> = (element: T, index: number, array: T[]) => boolean;
type MapCallBack<T, U> = (element: T, index: number, array: T[]) => U;

const mapper: MapCallBack<string | number, any> = (value, index, array) => {
...
const filterFunction: FilterCallBack<string | number> = (value, index, array) => {
  if (typeof value === 'string') {
...
function mapWithFilter<T, U>(array: T[], mapFunc: MapCallBack<T, U>, filterFunc: FilterCallBack<U>): U[] {
