...
console.log("1. Write code to get array of names from given array of users");
{
  function getUserNames(array) {
    return array.reduce((arrayOfNames, user) => {
...
{
  const sortedUsersByAgeDESC = [...users].sort((user1, user2) => user2.age - user1.age);
  const sortedUsersByAgeDESC1 = [...users].sort((user1, user2) => (user1.age < user2.age ? 1 : -1));
