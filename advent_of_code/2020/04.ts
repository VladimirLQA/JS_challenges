export const testPassportsList = `byr:2024 iyr:2016
...
export function isPassportValid(passport: string): boolean {
  ...
  return requiredFields
    .every((field) => passport.includes(field + ':'));
}

const fieldRules = {
  byr: (value: string) => {
    const v = value.trim();
    return v.length === 4 && Number(v) >= 1920 && Number(v) <= 2002;
  },
  iyr: (value: string) => {
    const v = value.trim();
    return v.length === 4 && Number(v) >= 2010 && Number(v) <= 2020;
  },
  eyr: (value: string) => {
    const v = value.trim();
    return v.length === 4 && Number(v) >= 2020 && Number(v) <= 2030;
  },
  hgt: (value: string) => {
    const v = value.trim();
    if (v.endsWith('cm')) {
  ...
  hcl: (value: string) => /^#[0-9a-f]{6}$/.test(value.trim()),
  ecl: (value: string) =>
    ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(value.trim()),
  pid: (value: string) => /^\d{9}$/.test(value.trim()),
  cid: (_value: string) => true,
};

export function isPassportValid2(passport: string): boolean {
  if (!passport.trim()) return false;
  const containsAllRequiredFields = Object.keys(fieldRules)
    .filter((field) => field !== 'cid')
    .every((field) => passport.includes(field + ':'));

  if (!containsAllRequiredFields) return false;
  const passportFieldValue = passport.split(/\n|\s/).filter(Boolean);

  return passportFieldValue.every((entry: string) => {
    const [field, value] = entry.split(':');
    const rules = fieldRules[field as keyof typeof fieldRules];
    if (!rules) throw new Error(`No existing rules for field: '${field}'`);
    return rules(value);
  });
}

export function validPassportsCount(input: string): number {
  const passports = input.split('\n\n').filter(Boolean);
  return passports.reduce((count, passport) =>
    (isPassportValid2(passport) ? count + 1 : count), 0);
}

console.log(validPassportsCount(testPassportsList));
