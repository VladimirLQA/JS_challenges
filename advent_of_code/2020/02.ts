...
const parsePolicy = (
  ...
  const match = policy!.match(/(\d+)|([a-zA-Z])/g)!;
  if (!match) throw new Error(`No match found with policy: ${policy}`);

  const [min, max, char] = match;
  if (!min || !max || !char) throw new Error(`Policy format is corrupted!\nPolicy: ${policy}`);

  return { min: Number(min), max: Number(max), char: char.toLowerCase() };
};

const getValidPasswordsCount = (list: string[]): number => {
  const splittedPolicyPassword = list.map((item: string) => item.split(':'));

  let countValid = 0;
  for (const [policy, password] of splittedPolicyPassword) {

    const { min, max, char } = parsePolicy(policy);
    const count = password.toLowerCase().trim().split('').filter((c: string) => c === char).length;
    if (count >= min && count <= max) countValid += 1;
...
console.log('1. valid passwords count > ', getValidPasswordsCount(testList));


const getValidPasswordsCount2 = (list: string[]): number => {
  const splittedPolicyPassword = list.map((item: string) => item.split(':'));

  let countValid = 0;
  for (const [policy, password] of splittedPolicyPassword) {

    const { min: minPosition, max: maxPosition, char } = parsePolicy(policy);
    const metPositionCount = password.toLowerCase().trim().split('')
      .filter((c: string, idx: number) =>
        c === char
          ? [maxPosition - 1, minPosition - 1].includes(idx)
          : false,
      ).length;

    if (metPositionCount === 1) countValid += 1;
  }

  return countValid;
};

console.log('2. valid passwords count > ', getValidPasswordsCount2(testList));
