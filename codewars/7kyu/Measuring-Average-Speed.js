...

{
  const calculateSpeed = (distance, time) => {
    const mS = 2.23694;
    const d = parseInt(distance, 10) * (distance.endsWith('km') ? 1000 : 1);
    const t = parseInt(time, 10) * (time.endsWith('min') ? 60 : 1);
...
{
  const calculateSpeed = (distance, time) => {
    const d = parseInt(distance, 10) * (distance.match(/km$/) ? 1000 : 1);
    const t = parseInt(time, 10) * (time.match(/min$/) ? 60 : 1);
